...
void nonogram_strip_init( CELLSTRIP_T* cellstrip )
{
    ...
    struct POSSIBLE_T** next_item_ptr = &cellstrip->possible_list;
    ...
    // Begin calculations
    calculate_potential( cellstrip, &next_item_ptr, cellstrip->n_regions, pattern, cellstrip->length );
...
*/
static void calculate_potential( const CELLSTRIP_T* cellstrip_data_ptr, struct POSSIBLE_T*** next_item_ptr, const uint16_t top_region, bool* pattern, uint16_t mod_length )
{
    ...
    if( top_region>0 )
    {
        ...
        for( uint16_t start=first_start; start<=last_start; start++ )
        {
            ...
            calculate_potential( cellstrip_data_ptr, next_item_ptr, top_region-1, pattern, start-1 );
    ...
    else
    {
        ...
        add_potential( next_item_ptr, pattern, cellstrip_data_ptr->length );
    }
...
static void add_potential( struct POSSIBLE_T*** next_item_ptr, bool* pattern, uint16_t length )
{
    ...
    **next_item_ptr = new_possible;
    *next_item_ptr = &(new_possible->next);
}
